            ...
            <Dialog as="div" class="relative z-40 lg:hidden" @close="sidebarOpen = false">
                ...
                <div class="fixed inset-0 flex z-40">
                    <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                        ...
                        <DialogPanel class="relative flex-1 flex flex-col max-w-xs w-full pb-4 bg-slate-600">
                            ...
                            <router-link to="/" @click="sidebarOpen = false"
                                ...
                                <div class="">
                                    <img class="h-24 mx-auto w-auto rounded-md" src="../assets/img/smkn2.png" alt="Your Company" />
                                </div>
                                <div class="mt-1.5 ml-0.5 font-bold text-xl text-gray-200"></div>
                            </router-link>
                            <nav class="mt-2 flex-shrink-0 h-full divide-y divide-gray-400 overflow-y-auto"
                                aria-label="Sidebar">
                                <div class="px-2 space-y-1">
                                    <a v-for="item in navigation" @click="sidebarOpen = false" :key="item.name"
                                        ...
                                        <component :is="item.icon" class="mr-4 flex-shrink-0 h-4 w-4 text-gray-200"
                                            aria-hidden="true" />
                                        {{ item.name }}
                                    </a>
                                </div>
            ...
            <div class="flex flex-col flex-grow bg-slate-600 pb-4 overflow-y-auto">
                <router-link to="/"
                    ...
                    <div class="flex gap-2 justify-center items-center">
                        <img class="w-12" src="../assets/img/smkn2.png" alt="Your Company" />
                        <h2 class="text-xl font-semibold text-white">Absensi Admin</h2>
                    </div>
                </router-link>
                <nav class="mt-2 flex-1 flex flex-col divide-y divide-gray-400 overflow-y-auto" aria-label="Sidebar">
                    <div class="px-2 space-y-1">
                        <a v-for="item in navigation" :key="item.name" :href="item.href" :class="[
                            ...
                            <component :is="item.icon" class="mr-2 flex-shrink-0 h-4 w-4 text-gray-200 hover:text-white"
                                aria-hidden="true" />
                            {{ item.name }}
                        </a>
                    </div>
                        ...
                        <Menu as="div" class="ml-3 relative">
                            <div>
                                <MenuButton
                                    ...
                                    <UserCircleIcon class="flex-shrink-0 ml-1 h-8 w-8 text-gray-400 lg:block"
                                        aria-hidden="true" />
                                    <span class="ml-3 text-gray-700 text-xs font-medium lg:block"><span class="sr-only">Open
                                            user menu for </span>Decy Alam
                                        Santoso</span>
                                    <ChevronDownIcon class="flex-shrink-0 ml-2 h-4 w-4 text-gray-400 lg:block"
                                        aria-hidden="true" />
                                </MenuButton>
                            </div>
